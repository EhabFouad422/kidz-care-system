...
export default function ReportsPage() {
  ...
  const loadStats = useCallback(async () => {
    ...
    const males = genders?.filter((p: any) => p.gender === 'male').length ?? 0
    const females = genders?.filter((p: any) => p.gender === 'female').length ?? 0
    ...
    if (format === 'csv') {
      const sections = [
        ...
        toCSV(['الكود','الاسم','الجنس','الميلاد','الأب','الهاتف','التسجيل'],
          (patients??[]).map((p: any) => [p.patient_number,p.full_name,genderAr(p.gender),fmtDate(p.date_of_birth),p.father_name??'',p.phone??'',fmtDate(p.created_at)])),
        '', '## الزيارات ##',
      ...
      printHTML(`
        ...
        <tbody>${(patients??[]).map((p: any)=>`<tr><td>${p.patient_number}</td><td><strong>${p.full_name}</strong></td>
